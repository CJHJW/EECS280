...
vector<double> mean_diff_sampling_distribution(
  ...
  for (int i = 0; i < 1000; i++) {
    vector<double> a = bootstrap_resample(data_A, i);
    vector<double> b = bootstrap_resample(data_B, i);
    double diff = mean(a) - mean(b);
    v.push_back(diff);
  }
  return v;
}
...
pair<double, double> confidence_interval(vector<double> v, double width) {
  ...
  double lower = percentile(v, (1-width)/2);
  double upper = percentile(v, 1-(1-width)/2);
  return {lower, upper};
}
